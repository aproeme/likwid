...
def write_pgf(group, test, plain_file, marker_file, scale=0.0,papi_file=None, execute=False, script=None):
    ...
    fp.write("\\addplot+[red,mark=square*,mark options={draw=red, fill=red}] table[x index=0, y index=2] {%s};\n" % (os.path.basename(plain_file),))
    ...
    fp.write("\\addplot+[green,mark=diamond*,mark options={draw=green, fill=green}] table[x index=0, y index=2] {%s};\n" % (os.path.basename(marker_file),))
    fp.write("\\addlegendentry{perfctr};\n")
    if papi and papi_file:
        fp.write("\\addplot+[black,mark=triangle*,mark options={draw=black, fill=black}] table[x index=0, y index=2] {%s};\n" % (os.path.basename(papi_file),))
